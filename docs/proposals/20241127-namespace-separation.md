...
- we don't need to support:
  - multiple versions of CRDs
  - RBAC !!TODO: ... why/what/...!!
- each instance can select the list of namespaces to watch
- a instance can select the list of namespaces to exclude from watching
...
No new team will be added. 

## Summary

https://cluster-api.sigs.k8s.io/developer/architecture/controllers/support-multiple-instances
...
- !!!TODO!!! Describe why the change is important and the benefits to users.
- !!!TODO!!! The motivation section can optionally provide links to [experience reports](https://github.com/golang/go/wiki/ExperienceReports)
to demonstrate the interest in a proposal within the wider Kubernetes community.

### Goals

Goals:
* add `--namespace=<ns1, …>` to extend `--namespace=<ns>` to watch on selected namespaces
  * this is only extending hash with one item to multiple items in the code
  * the maintenance complexity shouldn't be extended here 
* use `--excluded-namespace=<ecxl_ns1, …>` to define list of excluded namespaces
  * this is only using 

- !!!TODO!!! List the specific high-level goals of the proposal.
- !!!TODO!!! How will we know that this has succeeded?

### Non-Goals/Future Work

Non-goals:
* it's not necessary to work with the different versions of CRDs
* the contract and RBAC need to be solved on specific provider (AWS, AZURE, ...)

- !!!TODO!!! What high-levels are out of scope for this proposal?
- !!!TODO!!! Listing non-goals helps to focus discussion and make progress.
